...
func TestE2E_JapaneseLogicalNameIntegration(t *testing.T) {
	...
	t.Logf("   📊 Processed: %d tables, %d total columns", len(testSchema.Tables), len(testSchema.Tables[0].Columns))
	t.Logf("   🏷️  Logical names extracted: Tables: %s", testSchema.Tables[0].LogicalName)
		...
		Tables: []*schema.Table{
			{
				...
				Columns: []*schema.Column{
					...
					{Name: "column_2", Type: "VARCHAR(255)", Comment: "カラム2|テストデータのカラム2"},
				},
			},
			{
				Name:    "table_1",
